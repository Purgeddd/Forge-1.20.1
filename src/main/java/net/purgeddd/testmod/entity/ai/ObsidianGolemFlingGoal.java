...
public class ObsidianGolemFlingGoal extends Goal {
    ...
    private final int attackInterval = 20;  // Total ticks for the attack cycle

    public ObsidianGolemFlingGoal(ObsidianGolemEntity mob, double speedModifier, boolean followingTargetEvenIfNotSeen) {
        this.mob = mob;
    ...
    public void tick() {
        ...
        if (target != null) {
            ...
            if (distanceSq <= this.getAttackReachSqr(target)) {
                ...
                if (this.ticksUntilNextAttack == 6) {
